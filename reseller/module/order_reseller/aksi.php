...
if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])) {
      ...
      echo "<a href=../../index.php><b>LOGIN</b></a></center>";
        ...
        if(isset($_POST['tambah_item'])){   
            ...
            if($query) {
                echo"<script>alert('Item ditambahkan'); window.location = '$reseller_url'+'main.php?module=order_manual'; </script>"; 
            } else {
                echo "<script> alert('gagal'); window.location = '$reseller_url'+'main.php?module=tambah_item';</script>";
            }
            ...
            $query = mysqli_query($koneksi, "INSERT INTO orders (id_orders, status_order, tgl_order, jam_order, id_kustomer, id_reseller, level_order) VALUES ('$id_orders', 'Baru', '$tgl_order', '$jam_order', $id_kustomer, $id_reseller, '$level_order')");
            if($query) {
                unset($_SESSION['id_kustomer']);
                echo"<script> alert('Order berhasil!'); window.location = '$reseller_url'+'main.php?module=order'; </script>"; 
            } else {
                echo "<script> alert('Order gagal'); window.location = '$reseller_url'+'main.php?module=order_manual';</script>";
            }
           ...
           $jumlah = $_GET['jumlah'];
            ...
            if($query){
               echo "<script> window.location = '$reseller_url'+'main.php?module=order_manual';</script>";
            } else {
               echo "<script> window.location = '$reseller_url'+'main.php?module=order_manual';</script>"; 
            }
			...
			if ($query) {
				...
				echo "<script> alert('Ubah Status gagal'); window.location = '$reseller_url'+'main.php?module=order';</script>";
			}
