...
<?php
  ...
  <div class="card">
    <div class="card-header">
      ...
      <table id="example1" class="table table-bordered table-striped">
        ...
        <tbody>
          ...
          $id_reseller = $_SESSION['id_reseller'];
            $query = mysqli_query($koneksi, "SELECT c.id_orders as faktur,DATE_FORMAT(c.tgl_order, '%Y-%m-%d') as tanggal, nama_produk,jumlah,harga,level_order,harga_reseller FROM produk a JOIN orders_detail b ON a.id_produk=b.id_produk JOIN orders c ON b.id_orders=c.id_orders WHERE c.status_order='$status' AND c.id_reseller='$id_reseller' AND c.tgl_order BETWEEN '$mulai' AND '$selesai'");
            $i=1;
      ...
      <a class="btn btn-primary" href="module/laporan/content.php?mulai=<?= $mulai ?>&selesai=<?= $selesai ?>&status=<?= $status ?>&id_reseller=<?= $id_reseller ?>"><i class="fas fa-print"></i></a>  
    </div>
    </div>
  </div>
    <?php
